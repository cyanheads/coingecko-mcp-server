...
This server runs keyless by default — the one optional var attaches the Demo-tier header. `parseEnvConfig` reads an empty string and an unsubstituted MCPB `${user_config.X}` placeholder as unset, so a blank Claude Desktop field never sends a garbage header — no per-field `z.preprocess` guard.
...
await createApp({
  ...
  setup() { initCoinGeckoService(); },
  // teardown(core) { … }            // the setup() counterpart — unset here, nothing to release
});
```

This server declares `sessionMode: 'stateless'`: no tool calls `ctx.requestInput` and nothing is held per session. `MCP_SESSION_MODE` still wins whenever it carries a meaningful value (an explicit `auto` resolves to `stateful`); an empty string and an unsubstituted `${…}` placeholder read as unset and fall through to the declared `stateless`. A tool that asks the caller for input mid-handler needs `{ default: 'stateful', require: 'stateful' }` instead — startup then fails with a `ConfigurationError` rather than serving a mode in which a 2025-era client can never answer the prompt. Stdio is never refused.

`teardown(core)` is the `setup()` counterpart — release a watcher, socket, or non-`unref()`'d timer there. It runs after the transport stops and before the logger closes, on every shutdown path, and a signal-triggered shutdown then exits the process explicitly (0, or 1 if a step never settles within the framework's 10 s ceiling). The CoinGecko service allocates none of these, so this server sets no `teardown`.
...
This server centralizes its service-level reasons (`rate_limited`, `upstream_unreachable`, `invalid_response`) in `src/services/coingecko/error-contracts.ts` and spreads `...COINGECKO_SERVICE_ERRORS` into each tool's `errors[]`, alongside the per-tool reasons declared inline. Every shared entry carries `thrownBy: 'service'`, as does `coingecko_get_global`'s inline `unsupported_currency` (raised by `CoinGeckoService.global()`); every other declared reason is thrown by a literal `ctx.fail` in its handler, forwarding `ctx.recoveryFor`. The shared service contract is the one sanctioned exception to "don't extract a shared `errors[]` constant" — those reasons are identical across every tool because they all call the same HTTP client.
